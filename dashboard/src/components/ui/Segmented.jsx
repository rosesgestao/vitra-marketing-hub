...
export default function Segmented({ options = [], value, onChange, ariaLabel, size = 'md', className = '' }) {
  const pad = size === 'sm' ? 'px-2.5 py-1 text-2xs' : 'px-3 py-1.5 text-xs'
  return (
    <div role="group" aria-label={ariaLabel} className={`inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.02] p-1 ${className}`}>
      {options.map((opt) => {
